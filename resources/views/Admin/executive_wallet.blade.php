            ...
            <div class="row">
                <div class="col-lg-5 col-6 mx-auto">
                    <div class="small-box bg-info" style=" color: black;">
                        <div class="inner">
                            <h4>Executive Income Wallet</h4>
                            <p class="mb-0">
                                Total : {{ $executiveWalletTotal }}
                            </p>
                            <p class="mb-0">
                                <a href="{{ route('executiveUsersAmtList') }}" class="small-box-footer"
                                    style="color: black;">
                                    Redeemed : {{ $executiveWalletInactive }}
                                    <i class="fas fa-arrow-circle-right"></i>
                                </a>
                            </p>
                            <p>
                                Active : {{ $executiveWalletActive }}
                            </p>
                        ...
                        <div class="flex justify-around mt-2 pb-2" style="   display: flex; justify-content: space-between">

                            <a href="#" class="px-1 py-2 " style="color: black;" data-toggle="modal"
                                data-target="#executiveapproveModal">
        ...
        <div class="card recieptList col-md-11 mx-auto">
            ...
            <div class="card-body">
                <table id="example1" class="table table-bordered table-striped text-center">
                    ...
                    <tbody>
                        @forelse($executiveWallets as $index => $executiveWallet)
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                <td>{{ $executiveWallet->user->name ?? 'N/A' }}<br>{{ $executiveWallet->user->connection ?? '' }}
                                </td>
                                <td>{{ $executiveWallet->package->name ?? 'By Admin' }}</td>
                                <td>{{ $executiveWallet->amount }}</td>
                                <td>{{ $executiveWallet->created_at->format('d-m-Y') }}</td>
                                <td>
                                    @if ($executiveWallet->is_redeemed == 0)
                        ...
                        @empty
                            <tr>
                                <td colspan="6">No Executive users found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <div class="modal fade" id="companyapproveModal" tabindex="-1" aria-labelledby="companyapproveModal"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <form method="POST" action="{{ route('redeemExecutiveUsers') }}">
                    @csrf
                    <div class="modal-header">
                        <h5 class="modal-title" id="companyapproveModal">Confirm Executive income</h5>
                        <button type="button" class="close" data-dismiss="modal">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <p>Are you sure you want to redeem the amount to Executive users ?</p>
       ...
       <div class="modal fade" id="executiveapproveModal" tabindex="-1" aria-labelledby="executiveapproveModal"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <form method="POST" action="{{ route('redeemExecutiveCompany') }}">
                    @csrf
                    <div class="modal-header">
                        <h5 class="modal-title" id="executiveapproveModal">Confirm Executive Incentive</h5>
                        <button type="button" class="close" data-dismiss="modal">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
...
@section('footer')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            const walletLink = $('.nav-link.executivewallet');
