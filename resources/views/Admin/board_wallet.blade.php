...
@section('content')
            ...
            <div class="row">
                <div class="col-lg-5 col-6 mx-auto">
                    <div class="small-box bg-info" style=" color: black;">
                        <div class="inner">
                            <h4>Board Incentive Wallet</h4>
                            <p class="mb-0">
                                Total : {{ $boardWalletTotal }}
                            </p>
                            <p class="mb-0">
                                <a href="{{ route('boardUsersAmtList') }}" class="small-box-footer" style="color: black;">
                                    Redeemed : {{ $boardWalletInactive }}
                                    <i class="fas fa-arrow-circle-right"></i>
                                </a>
                            </p>
                            <p>
                                Active : {{ $boardWalletActive }}
                            </p>
        ...
        <div class="card recieptList col-md-11 mx-auto">
            ...
            <div class="card-body">
                <table id="example1" class="table table-bordered table-striped text-center">
                    ...
                    <tbody>
                        @forelse($boardWallets as $index => $boardWallet)
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                <td>{{ $boardWallet->user->name ?? 'N/A' }}<br>{{ $boardWallet->user->connection ?? '' }}
                                </td>
                                <td>{{ $boardWallet->package->name ?? 'By Admin' }}</td>
                                <td>{{ $boardWallet->amount }}</td>
                                <td>{{ $boardWallet->created_at->format('d-m-Y') }}</td>
                                <td>
                                    @if ($boardWallet->is_redeemed == 0)
                        ...
                        @empty
                            <tr>
                                <td colspan="6">No Board users found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <div class="modal fade" id="companyapproveModal" tabindex="-1" aria-labelledby="companyapproveModal"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <form method="POST" action="{{ route('redeemBoardUsers') }}">
                    @csrf
                    <div class="modal-header">
                        <h5 class="modal-title" id="companyapproveModal">Confirm Board Incentive</h5>
                        <button type="button" class="close" data-dismiss="modal">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <p>Are you sure you want to redeem the amount to Board users ?</p>
    ...
    <div class="modal fade" id="boardapproveModal" tabindex="-1" aria-labelledby="boardapproveModal"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <form method="POST" action="{{ route('redeemBoardCompany') }}">
                    @csrf
                    <div class="modal-header">
                        <h5 class="modal-title" id="boardapproveModal">Confirm Board Incentive</h5>
                        <button type="button" class="close" data-dismiss="modal">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
...
@section('footer')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            const walletLink = $('.nav-link.boardwallet');
