            ...
            <div class="row">
                <div class="col-lg-4 col-6 mx-auto">
                    <div class="small-box bg-info" style=" color: black;">
                        <div class="inner">
                            <h4>Royalty Income Wallet</h4>
                            <p class="mb-0">
                                Total : {{ $royaltyWalletTotal }}
                            </p>
                            <p class="mb-0">
                                <a href="{{route('royaltyUsersAmtList')}}" class="small-box-footer" style="color: black;">
                                    Redeemed : {{ $royaltyWalletInactive }}
                                    <i class="fas fa-arrow-circle-right"></i>
                                </a>
                            </p>
                            <p>
                                Active : {{ $royaltyWalletActive }}
                            </p>
        ...
        <div class="card recieptList col-md-11 mx-auto">
            ...
            <div class="card-body">
                <table id="example1" class="table table-bordered table-striped text-center">
                    ...
                    <tbody>
                        @forelse($royaltyWallets as $index => $royaltyWallet)
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                <td>{{ $royaltyWallet->user->name ?? 'N/A' }}<br>{{ $royaltyWallet->user->connection ?? '' }}
                                </td>
                                <td>{{ $royaltyWallet->package->name ?? 'By Admin'}}</td>
                                <td>{{ $royaltyWallet->amount }}</td>
                                <td>{{ $royaltyWallet->created_at->format('d-m-Y') }}</td>
                                <td>
                                    @if($royaltyWallet->is_redeemed == 0)
                        ...
                        @empty
                            <tr>
                                <td colspan="3">No royalty users found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <div class="modal fade" id="companyapproveModal" tabindex="-1" aria-labelledby="companyapproveModal"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <form method="POST" action="{{ route('redeemRoyaltyUsers') }}">
                    @csrf
                    <div class="modal-header">
                        <h5 class="modal-title" id="companyapproveModal">Confirm Royalty income</h5>
                        <button type="button" class="close" data-dismiss="modal">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <p>Are you sure you want to redeem the amount to royalty users ?</p>
...
@section('footer')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            const walletLink = $('.nav-link.royaltywallet');
