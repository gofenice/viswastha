    ...
    <div class="content-wrapper">
        ...
        @if (Auth::check() && Auth::user()->role != 'superadmin')
            <div class="card card-info col-md-8" style="margin: 0 auto;">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form class="form-horizontal" method="POST" action="{{ route('add_shop_receipt') }}"
                    enctype="multipart/form-data" id="user-pin-wallet">
                    {{-- onsubmit="disableSubmitButton() --}}
                    @csrf
                    <p data-user-url="{{ route('get_user_name') }}" id="wallet_name"></p>
                    <div class="card-body">
                        <div class="form-group row">
                            <label for="userId" class="col-sm-4 col-form-label">Owner Name</label>
                            <div class="col-sm-8">
                        ...
                        <div class="form-group row">
                            <label for="userName" class="col-sm-4 col-form-label">Shop Name</label>
                            <div class="col-sm-8">
                                <input type="text" class="form-control" value="{{ $shopdetail->name }}" name="userName"
                                    placeholder="" readonly>
                                <input type="hidden" class="form-control" name="shop_id" value="{{ $shopdetail->id }}">
                                @error('userName')
                    ...
                    <div class="card-footer">
                        <button type="submit" class="btn btn-info float-right" id="submitBtn">Send Wallet
                            Amount</button>
                    </div>
                    <!-- /.card-footer -->
                </form>
        ...
        <div class="card mt-3 recieptList col-md-11 mx-auto">
            ...
            <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                    ...
                    <tbody>
                        @if ($receipts)
                            @foreach ($receipts as $key => $reciept)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                        ...
                                        <td>
                                            ...
                                            @else
                                                <button class="btn btn-sm btn-primary" data-toggle="modal"
                                                    ...
                                                    data-status="{{ $reciept->status }}"
                                                    data-total-income="{{ $reciepts['totalIncomes'][$reciept->user_id] ?? 'N/A' }}"
                                                    data-amount="{{ number_format($reciept->amount, 2) }}">
                                                    Edit
                                                </button>
                                            @endif
                                        </td>
